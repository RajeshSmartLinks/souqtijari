                    ...
                    <table class="table zero-configuration">
                      ...
                      <tr>
                        ...
                        <td>{{$user->email}} </td>
							<!--DI CODE - End-->
                        <td>{{$user->mobile}} </td>
                        <td><a href="{{ route('user.edit', $user->id) }}"><i class="feather icon-edit"></i> Edit</a> | <a href="javascript:" onclick="onclickdelete({{$user->id}})" class="text-danger deleteBtn" data-id="{{$user->id}}" data-toggle="modal" data-target="#deleteModal" id="deleteBtn"><i class="feather icon-trash"></i> Delete</a> | 
						
						<a href="javascript:" onclick="onclickblock({{$user->id}})" id="blockclasscolor_{{$user->id}}" class="{{$user->status=='0' ? 'text-danger' : ''}}"><i class="feather {{$user->status=='0' ? 'icon-minus-circle' : 'icon-alert-circle'}}" id="blockclass_{{$user->id}}"></i><span id=blockcontent_{{$user->id}}> {{$user->status=='0' ? 'Blocked' : 'Not Blocked'}}</span></a></td>
                      </tr>
                      @endforeach
                      @else
