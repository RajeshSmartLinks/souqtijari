                    ...
                    <table class="table zero-configuration">
                      ...
                      <tr>
                        ...
                        <td>
							...
							<?php /*?> | <a href="javascript:" onclick="onclickblock({{$ad->id}})" id="blockclasscolor_{{$ad->id}}" class="{{$ad->status=='0' ? 'text-danger' : ''}}"><i class="feather {{$ad->status=='0' ? 'icon-minus-circle' : 'icon-alert-circle'}}" id="blockclass_{{$ad->id}}"></i><span id=blockcontent_{{$ad->id}}> {{$ad->status=='0' ? 'Blocked' : 'Not Blocked'}}</span></a> | 
							...
							<a href="javascript:" onclick="onclickpriority({{$ad->id}})"><i class="feather {{$ad->ad_priority ? 'icon-check-circle' : 'icon-circle'}}" id="priorityclass_{{$ad->id}}"></i><span id=prioritycontent_{{$ad->id}}> {{$ad->ad_priority ? 'Priority' : 'Not Priority'}}</span></a><?php */?>
						  </td>
                      </tr>
                      @endforeach
                      @else
